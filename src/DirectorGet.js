...
const Sidebar = () => {
    ...
    xhr.open("GET", `http://172.17.15.150:4000/AdminDirectorGet`, false);
    xhr.send();
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (token) {
                ...
                try {
                    const response = await fetch(`${BASE_URL}/DirectorDataKPIGet/${empId}/${activeTab}`);
                    const data = await response.json();
                    console.log(data, "dataaa-196");
    ...
    const handleSubTabClick = async (subTab) => {
        ...
        if (token) {
            ...
            try {
                const response = await fetch(`${BASE_URL}/DirectorDataKPIGet/${empId}/${activeTab}/${subTab}`);
                const data = await response.json();
    ...
    const handleSubTabUpdate = async () => {
        ...
        if (token) {
            ...
            try {
                ...
                const response = await fetch(`${BASE_URL}/DirectorDataKPIUpdate/${empId}/${activeTab}/${activeSubTab}`, {
                    method: 'PUT',
                    headers: {
