...
function ManagerUpdateEmpData() {
    ...
    const fetchInitialSections = async () => {
        try {
            const response = await axios.get(`${BASE_URL}/AdminEmployeeGet`);
    ...
    const handleItemClick = async (item) => {
        ...
        try {
            const response = await axios.get(`${BASE_URL}/EmployeeManagerDataKPIGet/${empId}/${selectedTitle}/${item}`);
            ...
            if (responseData) {
                ...
                try {
                    const newDataResponse = await axios.get(`${BASE_URL}/EmployeeDataKPIGet/${empId}/${selectedTitle}/${item}`);
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            try {
                ...
                try {
                    response = await axios.get(`${BASE_URL}/EmployeeDataKPIGet/${empId}/${selectedTitle}/${selectedItem}`);
                    responseData = response.data.data[0];
                } catch (error) {
                    responseData = null;
                }
    ...
    const handleSubmit = async () => {
        ...
        if (selectedItem) {
            ...
            try {
                const response = await axios.post(`${BASE_URL}/EmployeeManagerDataKPIPost`, JSON.stringify(postData), {
                    headers: {
    ...
    const updateApiData = async (newData) => {
        try {
            const response = await axios.put(
                `${BASE_URL}/EmployeeManagerDataKPIUpdate/${empId}/${selectedTitle}/${selectedItem}`,
                {
                    Data: newData,
                }
            );
