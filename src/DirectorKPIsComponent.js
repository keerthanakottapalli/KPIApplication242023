...
export default function DirectorKPIsComponent() {
  ...
  useEffect(() => {
    fetch(`${BASE_URL}/AdminDirectorGet`)
  ...
  const handleFormSubmit = (e) => {
    ...
    fetch(`${BASE_URL}/AdminDirectorPost`, {
      method: 'POST',
      headers: {
  ...
  const handleDeleteKPI = (e) => {
    ...
    let adminDirectorDeleteURL = `${BASE_URL}/AdminDirectorDelete/${adminId}`;

    if (isProcessKpiSelected) {
      adminDirectorDeleteURL += `/${selectedProcessKpi}`;
      if (isMetricSelected) {
        adminDirectorDeleteURL += `/${metric}`;
        if (isQuestionSelected) {
          adminDirectorDeleteURL += `/${question}`;
        }
      }
    }
  ...
  const handleProcessKpiChange = (_, newValue) => {
    ...
    if (apiData[newValue] && apiData[newValue][subTabValue]) {
      ...
      // Ensure that processKpi is set to the appropriate value
