...
const Sidebar = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (token) {
                ...
                try {
                    const response = await fetch(`${BASE_URL}/ManagerDataKPIGet/${empId}/${activeTab}`);
                    const data = await response.json();
    ...
    const handleSubTabClick = async (subTab) => {
        ...
        if (token) {
            ...
            try {
                const response = await fetch(`${BASE_URL}/ManagerDataKPIGet/${empId}/${activeTab}/${subTab}`);
                const data = await response.json();
    ...
    const handleSubTabUpdate = async () => {
        ...
        if (token) {
            ...
            try {
                ...
                const response = await fetch(`${BASE_URL}/ManagerDataKPIUpdate/${empId}/${activeTab}/${activeSubTab}`, {
                    method: 'PUT',
                    headers: {
                    ...
                    <div className='managertable' >
                        {loading ? (
                            ...
                            (activeSubTab && tabsData.length > 0 ? ( // Check if data is available
                                <>
                                    <TableContainer component={Paper} style={{ height: '43.5vh', overflow: "auto" }}>
                                        <Table style={{ overflow: "scroll", }}>
                                            <TableHead>
                                                <TableRow>
                                                    <TableCell className='tablecell-style' style={{  fontWeight: "bold", fontFamily: 'Open Sans,sans-serif!important' }}>Metric</TableCell>
                                                    <TableCell className='tablecell-style1' style={{  fontWeight: "bold", fontFamily: 'Open Sans,sans-serif!important', }}>Quantity Target</TableCell>
                                                    <TableCell className='tablecell-style2' style={{  fontWeight: "bold", fontFamily: 'Open Sans,sans-serif!important', }}>Quantity Achieved</TableCell>
                                                    <TableCell className='tablecell-style3' style={{  fontWeight: "bold", fontFamily: 'Open Sans,sans-serif!important', }}>Comments</TableCell>
                                                    <TableCell className='tablecell-style4' style={{  fontWeight: "bold", fontFamily: 'Open Sans,sans-serif!important', }}>Index KPI</TableCell>
                                                </TableRow>
                                            </TableHead>
                                            <TableBody>
